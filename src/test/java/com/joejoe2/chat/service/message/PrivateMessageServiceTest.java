...
class PrivateMessageServiceTest {
    ...
    assertThrows(
        ...
        () ->
            messageService.getAllMessages(
                userA.getId().toString(), PageRequest.builder().page(-1).size(0).build()));
    // test success
    // receive from user A
    SliceList<PrivateMessageDto> sliceList =
        messageService.getAllMessages(
            userA.getId().toString(), PageRequest.builder().page(1).size(4).build());
    assertTrue(sliceList.isHasNext());
    assertEquals(1, sliceList.getCurrentPage());
    assertEquals(4, sliceList.getPageSize());
    assertEquals(
        messages.subList(2, 6),
        sliceList.getList().stream().map(PrivateMessageDto::getContent).toList());
    sliceList =
        messageService.getAllMessages(
            userA.getId().toString(), PageRequest.builder().page(2).size(4).build());
    // receive from user C
  ...
  void getAllMessagesSince() throws Exception {
    ...
    assertThrows(
        ...
        () ->
            messageService.getAllMessages(
                userA.getId().toString(), since, PageRequest.builder().page(-1).size(0).build()));
    // test success
    // receive from user A
    SliceList<PrivateMessageDto> sliceList =
        messageService.getAllMessages(
            userA.getId().toString(), since, PageRequest.builder().page(1).size(4).build());
    assertTrue(sliceList.isHasNext());
    assertEquals(1, sliceList.getCurrentPage());
    assertEquals(4, sliceList.getPageSize());
    assertEquals(
    ...
    sliceList =
        messageService.getAllMessages(
            userA.getId().toString(), since, PageRequest.builder().page(2).size(4).build());
    // receive from user C
    assertFalse(sliceList.isHasNext());
    assertEquals(2, sliceList.getCurrentPage());
    assertEquals(4, sliceList.getPageSize());
