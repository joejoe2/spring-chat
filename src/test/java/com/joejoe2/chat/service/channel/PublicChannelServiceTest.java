...
class PublicChannelServiceTest {
  ...
  void createChannel() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> channelService.createChannel("invalid_name"));
    // test success
    assertDoesNotThrow(
        () -> {
  ...
  void getAllChannelsWithPage() throws Exception {
    ...
    assertThrows(
        ...
        () -> channelService.getAllChannels(PageRequest.builder().page(-1).size(0).build()));
    // test success
    assertEquals(
        channels.subList(10, 15),
        channelService.getAllChannels(PageRequest.builder().page(2).size(5).build()).getList());
  }
  ...
  void getChannelProfile() throws Exception {
    ...
    // test IllegalArgument
    assertThrows(
        IllegalArgumentException.class, () -> channelService.getChannelProfile("invalid id"));
